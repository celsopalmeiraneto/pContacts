...
(function($){
	...
	$.addFlex = function(t,p)
	{
		...
		$(g.bDiv).after(g.pDiv);
		var html = ' <div class="pGroup"> <div class="pFirst pButton"><span></span></div><div class="pPrev pButton"><span></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"><span class="pcontrol">P&aacute;gina <input type="text" size="4" value="1" /> de <span> 1 </span></span></div> <div class="btnseparator"></div> <div class="pGroup"> <div class="pNext pButton"><span></span></div><div class="pLast pButton"><span></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"> <div class="pReload pButton"><span></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"><span class="pPageStat"></span></div>';
		...
		if (p.searchitems)
			{
				...
				for (var s = 0; s < sitems.length; s++)
				{
					...
					if(sitems[s].cls==''){
						sClass = 'clrqsBoxMaskFlexigrid()';
					}else{
							sClass = 'pubqsBoxMaskFlexigrid()';
					}
					sopt += "<option value='" + sitems[s].name + "' " + sel + " onclick='"+sClass+"'>" + sitems[s].display + "&nbsp;&nbsp;</option>";						
				}
				
				if (p.qtype=='') p.qtype = sitems[0].name;
				
				$(g.sDiv).append("<div class='sDiv2'>"+p.searchmsg+"&nbsp;<input type='text' size='30' name='q' class='qsbox' /> <select name='qtype'>"+sopt+"</select> <input type='button' id='SearchButton' value='"+p.searchbtn+"' /><input type='button' id='clearSearchButton' value='"+p.searchclr+"' /></div>");

				$('input[name=q],select[name=qtype]',g.sDiv).keydown(function(e){if(e.keyCode==13) g.doSearch()});
				$('#SearchButton',g.sDiv).click(function(){g.doSearch()});
				$('#clearSearchButton',g.sDiv).click(function(){$('input[name=q]',g.sDiv).val('');p.query = '';g.doSearch();$('.qsbox').focus();});
				$(g.bDiv).after(g.sDiv);
				$('.qsbox').focus();
...
function pubqsBoxMaskFlexigrid(){
	$(function($){
		$('.qsbox').addClass("mask-data");
		$(".mask-data").mask("99/99/9999");
		$(".mask-hora").mask("99:99:99");
	});
}
function clrqsBoxMaskFlexigrid(){
	$(function($){
		$(".mask-data").unmask();
		$('.qsbox').focus();
	});
